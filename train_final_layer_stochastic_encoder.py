...
def update_model(rng, state, images, labels, mask0, mask1, mask2, mask3):
    ...
    def loss_fn(encoder_params, final_layers_params, images, labels):
        ...
        # noise_indicator = jnp.expand_dims(jax.random.bernoulli(key, p=noise_prob, shape=[batch_size]), axis=(0, -1))
        # # noisy_latent = jax.random.uniform(key, shape=z.shape, minval=-5, maxval=5)
        # noisy_latent = jnp.zeros(z.shape)
        # z = (1-noise_indicator)*z+noise_indicator*noisy_latent

        # z = z + jax.random.uniform(key, shape=z.shape, minval=-noise_prob, maxval=noise_prob)

        noise_prob= [mask0, mask1, mask2, mask3]
        z = z* np.expand_dims(1-np.array(noise_prob), axis=(0, 1))
...
@partial(jax.jit, static_argnums=(4,5,6,7))
def evaluate_model(key, state, images, labels, mask0, mask1, mask2, mask3):
    images = images.astype(jnp.float32) / 255.0
    encoder_state, final_layers_state = state
    z_dist = encoder_state.apply_fn(
        {
    ...
    # z = z + jax.random.uniform(key, shape=z.shape, minval=-noise_prob, maxval=noise_prob)
    noise_prob= [mask0, mask1, mask2, mask3]
    z = z* jnp.expand_dims(1-jnp.array(noise_prob), axis=(0, 1))
    qs = final_layers_state.apply_fn(
        {
    ...
    return loss, policy_avg_return

# @partial(jax.jit, static_argnums=(4,5,6,7))
# def get_model_outputs(key, state, images, labels, mask0, mask1, mask2, mask3):
#     images = images.astype(jnp.float32) / 255.0
#     encoder_state, final_layers_state = state
...
def create_state(learning_rate, rng):
    ...
    encoder_params = encoder_variables['params']
    encoder_tx = optax.adam(learning_rate)#optax.sgd(learning_rate, momentum)
    encoder_state = TrainState.create(apply_fn=encoder.apply,
                             params=encoder_params,
                             tx=encoder_tx)

    final_layers = NatureDQNNetwork2(action_dim=50)
    final_layers_variables = final_layers.init(rng, jnp.ones([1, latent_dim]))
    final_layers_params = final_layers_variables['params']
    final_layers_tx = optax.adam(learning_rate) #optax.sgd(learning_rate, momentum)
    final_layers_state = TrainState.create(apply_fn=final_layers.apply,
                             params=final_layers_params,
                             tx=final_layers_tx)
    return (encoder_state, final_layers_state)

def train_epoch(rng, state, train_ds, batch_size, epoch, noise_prob):
    ...
    for prm in tqdm.tqdm(permutations):
        ...
        rng, state, loss = update_model(rng, state, batch_images,
                                             batch_labels, noise_prob[0], noise_prob[1], noise_prob[2], noise_prob[3])
        metrics = {
            'loss': loss,
        }
...
def train(train_ds, test_ds, rotations, encoder_params, noise_prob):
    learning_rate = 1e-3
    batch_size = 128
    num_epochs = 10
    ...
    encoder_state, final_layers_state = state
    encoder_state = encoder_state.replace(params = encoder_params)
    state = (encoder_state, final_layers_state)


    for epoch in range(num_epochs):
        rng, input_rng = jax.random.split(rng)
        state = train_epoch(input_rng, state, train_ds, batch_size, epoch, noise_prob)
        # for rotation in rotations:
        #     rng, input_rng = jax.random.split(rng)
        #     test_loss, policy_avg_return = evaluate_model(input_rng, state, test_ds['image'+str(rotation)], test_ds['label'], noise_prob)
