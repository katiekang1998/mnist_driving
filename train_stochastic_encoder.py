...
def update_model(rng, state, images, labels, kl_coeff):
    ...
    def loss_fn(encoder_params, final_layers_params, images, labels):
        ...
        loss = jnp.mean(jnp.abs(jnp.reshape(qs, (batch_size, 10, 5)) - returns))
    ...
    loss_fn(encoder_state.params, final_layers_state.params, images, labels)
...
@jax.jit
def evaluate_model(key, state, images, labels):
    images = images.astype(jnp.float32) / 255.0
    encoder_state, final_layers_state = state
    z_dist = encoder_state.apply_fn(
        {
    ...
    best_actions = jnp.argmax(qs, axis=1)
    ...
    z = z_dist.sample(seed=key, sample_shape=100)

    #num samples x batch x z_dim
    ...
    best_robust_actions = jnp.argmax(jnp.percentile(qs, 2, axis=0), axis=1)
    ...
    return loss, policy_avg_return, robust_policy_avg_return, jnp.mean(jnp.std(z, axis=0))
...
def train(train_ds, test_ds, rotations, kl_coeff):
    learning_rate = 1e-3
    # momentum = 0.9
    batch_size = 128
    num_epochs = 20
    ...
    for epoch in range(num_epochs):
        ...
        for rotation in rotations:
            rng, input_rng = jax.random.split(rng)
            test_loss, policy_avg_return, robust_policy_avg_return, q_std = evaluate_model(input_rng, state, test_ds['image'+str(rotation)],
                                                  test_ds['label'])
